...
fn bench_real_world_parsing(c: &mut Criterion) {
    ...
    if files.is_empty() {
        ...
        let fallback_files = vec![
            ("small".to_string(), "config.json".to_string(), 
             r#"{"app": {"name": "test", "version": "1.0"}, "debug": true}"#.to_string()),
            ("medium".to_string(), "api_response.json".to_string(),
             format!("{{\"data\": [{}]}}", 
...
fn bench_parser_options_on_real_data(c: &mut Criterion) {
    ...
    for (category, filename, content) in test_files {
        for (option_name, options) in &parser_options {
            group.bench_with_input(
                BenchmarkId::new(
                    format!("{}_{}", category, option_name), 
                    filename.clone()
                ),
                &content,
                |b, json| {
                    b.iter(|| {
                        parse_with_options(black_box(json), options.clone())
                    })
                },
            );
        }
...
fn generate_mostly_objects_json(count: usize) -> String {
    let items: Vec<String> = (0..count)
        .map(|i| format!(r#"{{"id": {}, "name": "item{}", "value": {}, "active": {}}}"#, 
                        i, i, i * 10, i % 2 == 0))
        .collect();
    format!("[{}]", items.join(", "))
}
