...
fn bench_parse_small_object(c: &mut Criterion) {
    ...
    let inputs = vec![
        ...
        ("mixed types", r#"{"str": "text", "num": 42, "bool": true, "null": null}"#),
...
fn bench_parse_large_array(c: &mut Criterion) {
    ...
    let arrays = vec![
        ("10 elements", (0..10).map(|i| i.to_string()).collect::<Vec<_>>().join(", ")),
        ("100 elements", (0..100).map(|i| i.to_string()).collect::<Vec<_>>().join(", ")),
        ("1000 elements", (0..1000).map(|i| i.to_string()).collect::<Vec<_>>().join(", ")),
        ("mixed types 100", (0..100).map(|i| 
            match i % 4 {
                0 => i.to_string(),
                1 => format!("\"string_{}\"", i),
                2 => "true".to_string(),
...
fn bench_parse_with_options(c: &mut Criterion) {
    ...
    for (name, opts) in options {
        group.bench_with_input(
            ...
            |b, json| {
                b.iter(|| {
                    parse_with_options(black_box(json), opts.clone())
                })
            },
        );
    }
...
fn bench_parse_error_handling(c: &mut Criterion) {
    ...
    let deeply_nested = {
        let mut s = (0..200).fold(String::new(), |mut s, _| { s.push_str("{\"a\":"); s });
        s.push_str("1");
...
fn bench_parse_repair_mode(c: &mut Criterion) {
    ...
    let repair_options = vec![
        ("no_repair", ParserOptions {
            enable_repair: false,
            ..Default::default()
        }),
        ("repair_enabled", ParserOptions {
            enable_repair: true,
            max_repairs: 10,
            fast_repair: false,
            ..Default::default()
        }),
        ("fast_repair", ParserOptions {
            enable_repair: true,
            max_repairs: 10,
            fast_repair: true,
            ..Default::default()
...
fn bench_parse_string_heavy(c: &mut Criterion) {
    ...
    let string_types = vec![
        ("short_strings", (0..100).map(|i| format!(r#""str{}": "value{}""#, i, i)).collect::<Vec<_>>().join(", ")),
        ("long_strings", (0..10).map(|i| format!(r#""str{}": "{}""#, i, "x".repeat(100))).collect::<Vec<_>>().join(", ")),
        ("unicode_strings", (0..50).map(|i| format!(r#""str{}": "Hello 世界 {} 🌍""#, i, i)).collect::<Vec<_>>().join(", ")),
        ("escaped_strings", (0..50).map(|i| format!(r#""str{}": "line1\nline2\ttab\"quote\\slash""#, i)).collect::<Vec<_>>().join(", ")),
    ];

    for (name, content) in string_types {
        let json = format!("{{{}}}", content);
        group.bench_with_input(
            BenchmarkId::new("strings", name),
            &json,
            |b, json| {
                b.iter(|| {
                    parse(black_box(json))
