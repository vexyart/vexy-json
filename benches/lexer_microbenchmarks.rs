...
fn bench_tokenize_unquoted_keys(c: &mut Criterion) {
    ...
    let inputs = vec![
        ...
        ("mixed quoted/unquoted", r#"{key1: "value1", "key2": "value2"}"#),
        ("nested unquoted", r#"{outer: {inner: "value"}}"#),
        ("special chars", r#"{$key: "value", _key: "value", key123: "value"}"#),
    ];
...
fn bench_tokenize_numbers(c: &mut Criterion) {
    ...
    let inputs = vec![
        ...
        ("large numbers", "[9007199254740991, 1.7976931348623157e+308, 5e-324]"),
    ];
...
fn bench_tokenize_strings(c: &mut Criterion) {
    ...
    let long_strings = format!(r#"[{}]"#, 
        (0..100).map(|i| format!(r#""string_{}""#, i)).collect::<Vec<_>>().join(", ")
    );
    
    let inputs = vec![
        ("simple strings", r#"["hello", "world", "test"]"#),
        ("escaped strings", r#"["hello\nworld", "tab\there", "quote\"here"]"#),
        ("unicode strings", r#"["Hello 世界", "🚀✨🎉", "café"]"#),
        ("unicode escapes", r#"["\u0048\u0065\u006C\u006C\u006F", "\u4E16\u754C"]"#),
