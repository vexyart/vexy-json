...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_cases = vec![
        ...
        ("Array", r#"[1, 2, 3, 4, 5]"#),
        ("Nested", r#"{"user": {"profile": {"name": "Alice", "settings": {"theme": "dark"}}}}"#),
        ("With Comments", r#"{"key": "value", /* comment */ "number": 42}"#),
    ...
    for (name, json) in &test_cases {
        ...
        // Test all parsers
        let parsers: Vec<(&str, fn(&str, ParserOptions) -> Result<Value, vexy_json_core::Error>)> = vec![
        ...
        for (parser_name, parser_fn) in parsers {
            ...
            println!("  {} parser: {:?} ({} iterations)", parser_name, duration, 1000);
        ...
        match parser_fn(&large_json, options.clone()) {
            Ok(value) => {
                ...
                if let Value::Object(obj) = value {
                    println!("  {} parser: {:?} - {} keys ✅", parser_name, duration, obj.len());
                } else {
                    println!("  {} parser: {:?} - unexpected type ⚠️", parser_name, duration);
                }
            }
            Err(e) => {
