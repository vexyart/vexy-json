...
pub(super) fn parse_number_token(original_input: &str, span: Span) -> Result<Value> {
    ...
    let number_value = match normalized_number.parse::<f64>() {
        Ok(f) => {
            ...
            let in_range = f.is_finite() && f >= (i64::MIN as f64) && f <= (i64::MAX as f64) && (f as i64) as f64 == f;
            ...
            if has_no_fract && is_finite && in_range && !is_neg_zero && (!has_decimal_point || has_trailing_decimal) {
